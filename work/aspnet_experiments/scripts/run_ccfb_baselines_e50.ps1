param(
    ...
    [int]$Workers = 4,
    [switch]$IncludeSqrtBalancedSoftmax
)
...
$log = "logs/ccfb_baselines_e50_$stamp.log"
...
function Get-BalancedSoftmaxRunName {
    param([double]$ImbFactor)
    $ifTag = Format-ImbFactor $ImbFactor
    return "balanced_softmax_if${ifTag}_e$Epochs"
}

function Get-LdamRunName {
    param([double]$ImbFactor)
    $ifTag = Format-ImbFactor $ImbFactor
    return "ldam_drw_if${ifTag}_e$Epochs"
}
...
function Get-SqrtBalancedSoftmaxRunName {
    param(
        [double]$ImbFactor,
        [int]$Budget
    )
    $ifTag = Format-ImbFactor $ImbFactor
    return "adaptive_sqrt_budget${Budget}_if${ifTag}_e${Epochs}_bs"
}
...
function Run-Ce {
    ...
    Run-Step "CE IF=$ifTag e$Epochs" $runName @(
        ...
        "--model", "ce", "--loss", "ce", "--epochs", "$Epochs", "--batch-size", "$BatchSize",
        "--lr", "$Lr", "--workers", "$Workers", "--device", $Device, "--run-name", $runName
    )
}

function Run-BalancedSoftmax {
    param([double]$ImbFactor)
    $ifTag = Format-ImbFactor $ImbFactor
    $runName = Get-BalancedSoftmaxRunName $ImbFactor
    Run-Step "Balanced Softmax IF=$ifTag e$Epochs" $runName @(
        "train.py", "--dataset", $Dataset, "--data-root", $DataRoot, "--imb-factor", "$ifTag",
...
function Run-LdamDrw {
    param([double]$ImbFactor)
    $ifTag = Format-ImbFactor $ImbFactor
    $runName = Get-LdamRunName $ImbFactor
    Run-Step "LDAM-DRW IF=$ifTag e$Epochs" $runName @(
        "train.py", "--dataset", $Dataset, "--data-root", $DataRoot, "--imb-factor", "$ifTag",
        "--model", "ce", "--loss", "ldam", "--drw-epoch", "$DrwEpoch", "--epochs", "$Epochs",
        "--batch-size", "$BatchSize", "--lr", "$Lr", "--workers", "$Workers",
        "--device", $Device, "--run-name", $runName
    )
}
...
function Run-Fixed {
    ...
    Run-Step "Fixed K=$K IF=$ifTag e$Epochs" $runName @(
        "train.py", "--dataset", $Dataset, "--data-root", $DataRoot, "--imb-factor", "$ifTag",
...
function Run-Sqrt {
    ...
    $ifTag = Format-ImbFactor $ImbFactor
    if ($Loss -eq "balanced_softmax") {
        $runName = Get-SqrtBalancedSoftmaxRunName $ImbFactor $Budget
        $name = "Adaptive sqrt budget=$Budget + Balanced Softmax IF=$ifTag e$Epochs"
    } else {
        $runName = Get-SqrtRunName $ImbFactor $Budget
        $name = "Adaptive sqrt budget=$Budget IF=$ifTag e$Epochs"
    }

    Run-Step $name $runName @(
        "train.py", "--dataset", $Dataset, "--data-root", $DataRoot, "--imb-factor", "$ifTag",
        "--model", "adaptive_proto", "--allocation", "sqrt", "--proto-budget", "$Budget",
        "--k-min", "$KMin", "--k-max", "$KMax", "--loss", $Loss, "--epochs", "$Epochs",
        "--batch-size", "$BatchSize", "--lr", "$Lr", "--workers", "$Workers",
...
foreach ($imb in $ImbFactors) {
    Run-Ce $imb
    $summaryNames += Get-CeRunName $imb

    Run-BalancedSoftmax $imb
    $summaryNames += Get-BalancedSoftmaxRunName $imb

    Run-LdamDrw $imb
    $summaryNames += Get-LdamRunName $imb

    if (Test-IsIf100 $imb) {
        Run-Fixed $imb 4
        $summaryNames += Get-FixedRunName $imb 4
        Run-Sqrt $imb 200
        $summaryNames += Get-SqrtRunName $imb 200
        if ($IncludeSqrtBalancedSoftmax) {
            Run-Sqrt $imb 200 "balanced_softmax"
            $summaryNames += Get-SqrtBalancedSoftmaxRunName $imb 200
        }
    } else {
        Run-Fixed $imb 3
        $summaryNames += Get-FixedRunName $imb 3
        Run-Fixed $imb 4
        $summaryNames += Get-FixedRunName $imb 4
        Run-Sqrt $imb 300
        $summaryNames += Get-SqrtRunName $imb 300
        if ($IncludeSqrtBalancedSoftmax) {
            Run-Sqrt $imb 300 "balanced_softmax"
            $summaryNames += Get-SqrtBalancedSoftmaxRunName $imb 300
        }
    }
}
