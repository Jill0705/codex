...
$log = "logs/ldam_diagnostics_e10_$stamp.log"
...
$summaryNames = @()
$ifArg = Format-NumberArg $ImbFactor

foreach ($scale in $Scales) {
    $scaleArg = Format-NumberArg $scale
    $scaleTag = Format-NumberTag $scale
    $runName = "ldam_drw_if100_e${Epochs}_s${scaleTag}"
    $summaryNames += $runName

    Run-Step "LDAM diagnostic IF=$ifArg scale=$scaleArg e$Epochs" $runName @(
...
foreach ($n in $summaryNames) {
    ...
    if (Test-Path $p) {
        ...
        $last = $rows | Select-Object -Last 1
        $msg = "{0}: best_epoch={1}, best_val={2}, last_train_acc={3}, many={4}, medium={5}, few={6}" -f $n, $best.epoch, $best.val_acc, $last.train_acc, $best.many_acc, $best.medium_acc, $best.few_acc
        Write-Host $msg
        Add-Content -Path $log -Value $msg
    } else {
